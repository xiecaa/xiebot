...
async function starts() {
   ...
   xiedev.on('message-update', async (team) => {
		try {
	   const from = team.key.remoteJid
		const messageStubType = WA_MESSAGE_STUB_TYPES[team.messageStubType] || 'MESSAGE'
		...
		const sender = team.key.fromMe ? xiedev.user.jid : team.key.remoteJid.endsWith('@g.us') ? team.participant : team.key.remoteJid
		const isRevoke = team.key.remoteJid.endsWith('@s.whatsapp.net') ? true : team.key.remoteJid.endsWith('@g.us') ? dataRevoke.includes(from) : false
		const isCtRevoke = team.key.remoteJid.endsWith('@g.us') ? true : dataCtRevoke.data ? true : false
		const isBanCtRevoke = team.key.remoteJid.endsWith('@g.us') ? true : !dataBanCtRevoke.includes(sender) ? true : false
		if (messageStubType == 'REVOKE') {
			...
			const from = team.key.remoteJid
			const isGroup = team.key.remoteJid.endsWith('@g.us') ? true : false
			let int
			let infoMSG = JSON.parse(fs.readFileSync('./database/.dat/msg.data.json'))
			const id_deleted = team.key.id
			...
			if (int.type == 'conversation' || int.type == 'extendedTextMessage') {
				const strConversation = `╭${fx}「 ANTIDELETE 」
├ Nama : *${pushname}*
...
└ Pesan : *${body ? body : '-'}*`
				xiedev.sendMessage(from, strConversation, MessageType.text, {quoted: {key: {fromMe:false, participant:`6283818221226@s.whatsapp.net`, ...(from ? {remoteJid :"status@broadcast" }: {}) },message:{"orderMessage":{"orderId":"174238614569481","thumbnail": fs.readFileSync('./src/fotobot.jpg'),"itemCount":29,"status":"INQUIRY","surface":"CATALOG","message": namabot,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {forwardingScore: 508, isForwarded: true}})
				...
				const savedFilename = await xiedev.downloadAndSaveMediaMessage(int.data, `./media/sticker/${filename}`);
				const strConversation = `╭${fx}「 ANTIDELETE 」
├ Nama : *${pushname}*
├ Nomer : ${sender.replace('@s.whatsapp.net', '')}
├ Tipe : Text
...
└ Tanggal : *${moment.unix(int.timestamp).format('DD/MM/YYYY')}*`
				...
				xiedev.sendMessage(from, strConversation, MessageType.text, {quoted: {key: {fromMe:false, participant:`6283818221226@s.whatsapp.net`, ...(from ? {remoteJid :"status@broadcast" }: {}) },message:{"orderMessage":{"orderId":"174238614569481","thumbnail": fs.readFileSync('./src/fotobot.jpg'),"itemCount":29,"status":"INQUIRY","surface":"CATALOG","message": namabot,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {forwardingScore: 508, isForwarded: true}})
				xiedev.sendMessage(from, buff, MessageType.sticker, {quoted: {key: {fromMe:false, participant:`6283818221226@s.whatsapp.net`, ...(from ? {remoteJid :"status@broadcast" }: {}) },message:{"orderMessage":{"orderId":"174238614569481","thumbnail": fs.readFileSync('./src/fotobot.jpg'),"itemCount":29,"status":"INQUIRY","surface":"CATALOG","message": namabot,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {forwardingScore: 508, isForwarded: true}})
				...
				const filename = `${sender.replace('@s.whatsapp.net', '')}-${moment().unix()}`
				const savedFilename = await xiedev.downloadAndSaveMediaMessage(int.data, `./media/revoke/${filename}`);
				const buff = fs.readFileSync(savedFilename)
				const strConversation = `╭${fx}「 ANTIDELETE 」
├ Nama : *${pushname}*
├ Nomer : ${sender.replace('@s.whatsapp.net', '')}
...
└ Pesan : ${body ? body : '-'}`
				...
				const filename = `${sender.replace('@s.whatsapp.net', '')}-${moment().unix()}`
				const savedFilename = await xiedev.downloadAndSaveMediaMessage(int.data, `./media/video/${filename}`);
				const buff = fs.readFileSync(savedFilename)
				const strConversation = `╭${fx}「 ANTIDELETE 」
├ Nama : *${pushname}*
├ Nomer : ${sender.replace('@s.whatsapp.net', '')}
...
├ Tanggal : *${moment.unix(int.timestamp).format('DD/MM/YYYY')}*
└ Pesan : ${body ? body : '-'}`
				xiedev.sendMessage(from, buff, MessageType.video, {quoted: {key: {fromMe:false, participant:`6283818221226@s.whatsapp.net`, ...(from ? {remoteJid :"status@broadcast" }: {}) },message:{"orderMessage":{"orderId":"174238614569481","thumbnail": fs.readFileSync('./src/fotobot.jpg'),"itemCount":29,"status":"INQUIRY","surface":"CATALOG","message": namabot,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, mimetype: 'video/mp4', contextInfo: {forwardingScore: 508, isForwarded: true, mentionedJid: [sender]}, caption: strConversation })
				fs.unlinkSync(savedFilename) 
			}
		}
	} catch (e) {
	...
	xiedev.on('message-new', async (team) => {
		   try {
			if (!team.hasNewMessage) return
			team = team.messages.all()[0]
			if (team.key && team.key.remoteJid == 'status@broadcast') return
         let infoMSG = JSON.parse(fs.readFileSync('./database/.dat/msg.data.json'))
         infoMSG.push(JSON.parse(JSON.stringify(team)))
         fs.writeFileSync('./database/.dat/msg.data.json', JSON.stringify(infoMSG, null, 2))
...
`
              ...
              [
                 ...
                 {buttonId:`${prefix}daftarinfo`,buttonText:{displayText:'DAFTAR INFO'},type:1}
         ...
         case 'daftarinfo':
              ...
              xiedev.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: {mentionedJid: [sender]}, quoted:ftroli})
              break
//>>>>>>>>>[ KHUSUS INFO BOT ]<<<<<<<<<<\\
         case 'owner':
         case 'creator':
...
//>>>>>>>>>[ KHUSUS GROUP ]<<<<<<<<<<\\
         ...
         case 'antidelete': 
              ...
              if (argz[1] == 'aktif') {
              	...
              	if (argz.length === 2 || argz[2].startsWith('0')) return reply(`Masukan nomer diawali dengan 62! contoh 62838159xxxxx`)
              	dataBanCtRevoke.push(argz[2] + '@s.whatsapp.net')
              	fs.writeFileSync('./database/ct-revoked-banlist.json', JSON.stringify(dataBanCtRevoke, null, 2))
              	reply(`Kontak ${argz[2]} telah dimasukan ke banlist antidelete secara permanen!`)
              } else if (argz[1] == 'mati') {
              	if (isGroup) {
...
//>>>>>>>>>[ KHUSUS DOWNLOAD ]<<<<<<<<<<\\
         case 'play':
              ...
              const megnya = await xiedev.prepareMessage(from, res.all[0].image, location, {thumbnail: res.all[0].thumb})
        ...
        case 'tiktokmusic':
        case 'ttmp3':
              if (!q) return reply(`Contoh: ${prefix + command} link video tiktok`)
              reply(mess.wait)
              hx.ttdownloader(`${args[0]}`)
              .then(async result => {
              console.log('Mendownload Video TikTok')
              const { wm, nowm, audio } = result
              kyyyy ='vt.mp4'
              let kntl = 'att.mp3'
              attnya = 'att.mp3'
              buper = await getBuffer(nowm);
              console.log('Mengconvert Ke Audionya...')
              fs.writeFileSync(`${kyyyy}`, buper);
              exec(`ffmpeg -i vt.mp4 -vn \ -acodec libmp3lame -ac 2 -ab 160k -ar 48000 \ att.mp3`,(err,res)=> {
              console.log('Sukses Mengirim Audionya')
              if (err) return reply(mess.error.eror)
              })
              setTimeout( () => {
              const ttkmp3 = fs.readFileSync('att.mp3')
              xiedev.sendMessage(from, ttkmp3, MessageType.audio, {mimetype: 'audio/mp4', quoted: team})
              }, 6000)
              })
              fs.unlinkSync(attnya)
              break
         case 'mediafire':
         case 'mdf':
              if (!isUser) return replyy(from, daftar1, daftar2, daftar3, {quoted: ftroli})
