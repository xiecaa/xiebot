	...
	xiedev.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
			   ...
			   xiedev.sendMessage(mdata.id, halo, MessageType.audio, {contextInfo: {"mentionedJid": [num]}, contextInfo: {"forwardingScore": 1945, "isForwarded": true, "externalAdReply": {"title": `Welcome @${num.split('@')[0]}`,"body": `${authorbot}`,"mediaType": 2,"thumbnailUrl": buffer,"mediaUrl": "","thumbnail": buffer}}, mimetype: 'audio/mp4', ptt: true, seconds: 99999999})
				...
				let buffer = await getBuffer(ppimg)
				xiedev.sendMessage(mdata.id, halo, MessageType.audio, {contextInfo: {"mentionedJid": [num]}, contextInfo: {"forwardingScore": 1945, "isForwarded": true, "externalAdReply": {"title": `Sayonara @${num.split('@')[0]}`,"body": `${authorbot}`,"mediaType": 2,"thumbnailUrl": buffer,"mediaUrl": "","thumbnail": buffer}}, mimetype: 'audio/mp4', ptt: true, seconds: 99999999})
			}
		} catch (e) {
...
//>>>>>>>>>[ KHUSUS INFO BOT ]<<<<<<<<<<\\
         ...
         case 'rulesbot':
              ...
              sendButtLocation(from, `${teksnya}`, `${authorbot}`, {jpegThumbnail: fs.readFileSync('./src/fotobot.jpg')}, 
              [
                 {buttonId:`${prefix}owner`,buttonText:{displayText:'OWNER'},type:1}
            ...
            const fduc = {
					"productMessage": {
						"product": {
							...
							"productId": "4559966904061216",
							"title": "Hai Owner",
							"description": "Tes1 Done Kak!",
							"currencyCode": "IDR",
							"priceAmount1000": "1000",
							"productImageCount": 1
						},
						"businessOwnerJid": "6283873517269@s.whatsapp.net"
					}
				}
				res = await xiedev.prepareMessageFromContent(from, fduc, {quoted: ftroli, contextInfo: {"mentionedJid": [sender]}})
