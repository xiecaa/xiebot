         ...
         const sendStickerFromUrl = async(to, url) => {
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Jakarta').format('HH:mm:ss'), "gold"), color('Downloading sticker...'))
				...
				download(url, namea, async function () {
					...
					xiedev.sendMessage(to, media, sticker)
...
//>>>>>>>>>[ KHUSUS INFO BOT ]<<<<<<<<<<\\
         ...
         case 'ping':
              ...
              xiedev.sendMessage(from, `*Speed* : ${latensi.toFixed(4)}`, text, {quoted: team})
              break
         case 'info':
              if (!isUser) return replyy(from, daftar1, daftar2, daftar3, {quoted: ftroli})
              if (isBanned) return replyy(from, banned1, banned2, banned3, {quoted: ftroli})
              ...
              randKey = jsonData[randXiedev];
              buffer = await getBuffer(randKey.image)
              imegnya = await xiedev.prepareMessage(from, buffer, image, {thumbnail: buffer})
              ggbutsan = [
                {buttonId: `${prefix}owner`, buttonText: {displayText: 'OWNER'}, type: 1}
              ]
              gbuttonan = {
                imageMessage: imegnya.message.imageMessage,
                contentText: `${teksnya}`,
                footerText: `${authorbot}`,
                buttons: ggbutsan,
                headerType: 4
              }
              await xiedev.sendMessage(from, gbuttonan, MessageType.buttonsMessage, {thumbnail: fs.readFileSync('./src/fotobot.jpg'), contextInfo: {mentionedJid: [sender]}, quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: `${namabot}`, orderTitle: `${namabot}`, thumbnail: buffer, sellerJid: '0@s.whatsapp.net'}}}})
              break
         case 'rules':
         case 'rulesbot':
              if (!isUser) return replyy(from, daftar1, daftar2, daftar3, {quoted: ftroli})
...
//>>>>>>>>>[ KHUSUS DOWNLOAD ]<<<<<<<<<<\\
         case 'play':
              ...
              xiedev.sendMessage(from, buttsss, MessageType.buttonsMessage,{quoted: team, contextInfo: {"mentionedJid": [sender]}})
	           break
         case 'play2':
              if (!isUser) return replyy(from, daftar1, daftar2, daftar3, {quoted: ftroli})
              if (isBanned) return replyy(from, banned1, banned2, banned3, {quoted: ftroli})
         ...
         case 'asupan':
              ...
              const butt = [
                    ...
                    {buttonId:`${prefix}tomp3`,buttonText:{displayText:'AUDIO'},type:1}
              ]
              const buttonMessages = {
         ...
         case 'tes':
              ...
              const ginibang = {
					"extendedTextMessage": {
						...
						"matchedText": "",
						"canonicalUrl": "",
						"description": "Done Kak",
						"title": "HALO OWNER KU",
						"previewType": "VIDEO",
						"jpegThumbnail": fs.readFileSync('./src/fotobot.jpg')
					  }
				  }
				  ress3 = await xiedev.prepareMessageFromContent(from, ginibang, {quoted: ftroli, contextInfo: {"mentionedJid": [sender]}})
              xiedev.relayWAMessage(ress3)
              break
         case 'tes2':
              if (!isOwner && !team.key.fromMe) return reply(mess.only.ownerB)
              data = fs.readFileSync('./lib/image.js');
